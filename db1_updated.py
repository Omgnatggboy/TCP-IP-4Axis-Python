...
def WaitArrive(target):
    """รอจนกว่าจะถึงจุดหมาย (Cartesian Point) พร้อม Debug Log"""
...
def movj_wait(point, description=""):
...
def pick_and_place(to_conveyor, relative_coords=None):
    ...
    movj_wait(HOME_POINT, "Moving to HOME")
    
    # Step 1: ไปเหนือวัตถุ
    approach_pos = [ox, oy, oz + 20, orot]
    movj_wait(approach_pos, "Approach Above Object")
    ...
    movj_wait(HOME_POINT, "Transit to HOME")
    movj_wait(target_dest, "Moving to DROP Point")
    
    suction_release()
    movj_wait(HOME_POINT, "Task Complete - Back to HOME")
...
def edge_to_edge_sweep(relative_coords):
    ...
    movj_wait(HOME_POINT, "Move to HOME for Sweep Start")
    movj_wait([target_x, y_start, oz + 25, orot], "Positioning at Sweep Edge")
    ...
    movj_wait(HOME_POINT, "Sweep Finished - Back to HOME")
